...
public:
	...
	bool Load(std::wstring sFile)
	{
		...
		int byteCount = nWidth * nHeight;

		if (byteCount > sizeof(m_Colours))
		{
			return false;
		}

		std::fread(m_Colours, sizeof(short), byteCount, f);

		if (byteCount > sizeof(m_Glyphs))
		{
			return false;
		}
		std::fread(m_Glyphs, sizeof(short), byteCount, f);
