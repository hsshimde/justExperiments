...
Vector3D Vector3D::MultiplyMatrix(const Matrix4D& mat) const
{
	...
	result.mfX = mfX * mat.m[0][0] + mfY * mat.m[0][1] + mfZ * mat.m[0][2] + mfW * mat.m[0][3];
	result.mfY = mfX * mat.m[1][0] + mfY * mat.m[1][1] + mfZ * mat.m[1][2] + mfW * mat.m[1][3];
	result.mfZ = mfX * mat.m[2][0] + mfY * mat.m[2][1] + mfZ * mat.m[2][2] + mfW * mat.m[2][3];
	result.mfW = mfX * mat.m[3][0] + mfY * mat.m[3][1] + mfZ * mat.m[3][2] + mfW * mat.m[3][3];

	return result;

}
void Vector3D::AddScreenOffset(const float offset)
{
	mfZ += offset;
	return;
}
